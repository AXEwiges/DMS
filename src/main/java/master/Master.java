  ...
  public static void main(String[] args)
    ...
    Client masterClient = new ClientMasterImpl(new ClientConnectionStrategy() {
      ...
      public void onConnect(ClientInfo clientInfo) {
        try {
          System.out.println(
              clientInfo + " connected, uid = " + clientInfo.uid);
          if (!regionsInfomation.containsKey(clientInfo.uid)) {
            ...
            List<String> tables = regionsToTables.get(
                clientInfo.uid); //tables存放新建region存有的表list
            for (Integer uid : regionsToTables.keySet()) {
              ...
              try {
                Region.Client client = ThriftClient.getForRegionServer(
                    source_ip, 5099);
                if (regionsToTables.get(uid).size() > avg) {
                  for (int j = 0; j < regionsToTables.get(uid).size() - avg;
                      j++) {
                    ...
                    if (!tables.contains(tableName)) {
                      ...
                      client.requestCopyTable(
                          clientInfo.ip + ":" + clientInfo.socketPort,
                          tableName, true);
                    }
                      if (tables.size() >= avg) {
                          break;
      ...
      public void onDisconnect(ClientInfo clientInfo) {
        try {
          ...
          for (String tableName : list) {
            ...
            Region.Client client = ThriftClient.getForRegionServer(source_ip,
                5099);
